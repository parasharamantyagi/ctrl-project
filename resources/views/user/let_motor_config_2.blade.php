...
<!-- h5>Led motor config</h5 -->
	<div class="btn-firs-toggle">
		...
		<div class="button-option-ceil" style="display: none;">
			...
			<p><button class="btn btn-secondary led-external-board-id">LED EXTERNAL BOARD ID</button></p>
		</div>
	<div>
	...
	<script src="https://rvera.github.io/image-picker/image-picker/image-picker.js" type="text/javascript"></script>
		...
		$(document).ready(function(){
			...
			function entranceAjaxFunction(sequence_name){
				...
				$('div[class="entrance_on_mode_div"]').html('');
				$('div[class="entrance_off_mode_div"]').html('');
			...
			$('button[class="btn btn-secondary undo"]').on("click", function(event){
				$.ajax({
					url: "entrance-west-building-clone",
					type: 'POST',
					dataType: "JSON",
			...
			$('button[class="btn btn-secondary entrance-motor-config"]').on("click",function(e){
				location.href = "/user/led-motor-excel-sheet";
			});
			
			$('button[class="btn btn-secondary led-external-board-id"]').on("click",function(e){
				location.href = "/user/led-external-board-id?vehicle_id="+$('input[name="vehicle_id"]').val();
			});
